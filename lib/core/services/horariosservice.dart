...
class HorariosService{
  static Future<Map<String, dynamic>> index({@required BuildContext context, scaffoldKey}) async {
    ...
    if(statusCode < 200 || statusCode > 400){
      print("HorariosService index: ${response.body}");
      ...
      throw Exception("Error del servidor HorariosService index: ${parsed["message"]}");
    }

    var parsed = await compute(Utils.parseDatos, response.body);
      print("HorariosService index parsed: ${parsed}");
    ...
    if(parsed["errores"] == 1){
      ...
      throw Exception("Error HorariosService index: ${parsed["mensaje"]}");
    }
  ...
  static Future<Map<String, dynamic>> guardar({@required BuildContext context, scaffoldKey, @required List<Loteria> data}) async {
    var map = Map<String, dynamic>();
    var mapDatos = Map<String, dynamic>();

    map["idUsuario"] = await Db.idUsuario();
    map["loterias"] = Loteria.loteriasToJson(data);
    map["servidor"] = await Db.servidor();
    var jwt = await Utils.createJwt(map);
    // mapDatos["datos"] = jwt;
    mapDatos = {
    ...
    if(statusCode < 200 || statusCode > 400){
      print("HorariosService guardar: ${response.body}");
      var parsed = await compute(Utils.parseDatos, response.body);
      if(context != null)
        Utils.showAlertDialog(context: context, content: "${parsed["message"]}", title: "Error");
      else
        Utils.showSnackBar(content: "${parsed["message"]}", scaffoldKey: scaffoldKey);
      throw Exception("Error del servidor HorariosService guardar ${parsed["message"]}");
    }
    ...
    if(parsed["errores"] == 1){
      if(context != null)
        ...
        Utils.showSnackBar(content: parsed["mensaje"], scaffoldKey: scaffoldKey);
      throw Exception("Error HorariosService guardar: ${parsed["mensaje"]}");
    }
  ...
  static Future<Map<String, dynamic>> eliminar({@required BuildContext context, scaffoldKey, int idUsuario, @required Grupo grupo}) async {
    ...
    if(statusCode < 200 || statusCode > 400){
      ...
      if(context != null)
        ...
        Utils.showSnackBar(content: "Error del servidor HorariosService guardar ${parsed["message"]}", scaffoldKey: scaffoldKey);
      throw Exception("Error del servidor HorariosService guardar: ${parsed["message"]}");
    }
